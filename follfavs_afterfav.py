...
writer = csv.writer(open("future-favorites.csv", "wb"), delimiter='\t')
...
for dest in favorites:
    for src, favdate in favorites[dest]:
        ...
        for favoriter, favoritedate in favorites[dest]:
            if favoritedate > favdate:
                totalfavcount += 1
                if favoriter in twohopfoll:
                    twohopfavcount += 1
                    converts.add(favoriter)
        ...
        writer.writerow([dest, len(destfoll), src, len(srcfoll), totalfavcount, twohopfavcount, len(twohopfoll),
                         percentage])
